...
export function Header() {
  ...
  return <header className={scrolled ? 'header scrolled' : 'header'}><div className="container header-inner"><Brand/>
    <nav className="desktop-nav" aria-label="Основне меню">{navigation.map((group) => <div className="desktop-nav-group" key={group.label}><button type="button" aria-expanded={activeGroup === group.label} data-active={isGroupActive(path, group) || undefined} onClick={() => setActiveGroup(activeGroup === group.label ? null : group.label)}>{group.label}<ChevronDown size={14}/></button>{activeGroup === group.label && <div className="desktop-nav-panel">{group.items.map((item) => <Link href={item.href} key={item.href}><strong>{item.label}</strong><small>{item.description}</small></Link>)}</div>}</div>)}</nav>
